...
class Regex2(Regex):        
    ...
    def init_name(self):
        self.name = Regex2.build_name(self.prefix_name)
        self.name_properties = self.name+str(self.queries_sw)+"_"+str(self.articles_sw)
    
    @staticmethod
    def build_name(prefix_name):
        return prefix_name + "_Regex2Tokenizer"
    ...
    def load_from_json(**kwargs):
        if "path" in kwargs:
            ...
            path = join(kwargs["cache_folder"], Regex2.build_name(kwargs["prefix_name"])+".json")
        else:
            raise ValueError("Path not specified")
